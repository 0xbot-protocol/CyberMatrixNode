...
class StringUtil
{
    ...
    public static function getInviteCode($len = 6) {
    ...
    public static function humanDate($o) {
    ...
    public static function getAccessToken ($app)
    {
        ...
        // 使用uniqid()方法创建唯一i
        $request_id = $app.md5(uniqid($prefix, true).self::getUUid());
        // 格式化请求id
        return $request_id;
    }
    ...
    public static function getUUid()
    {
        ...
        $workerId = ServerManager::getInstance()->getSwooleServer()->worker_id??0;

        echo $dataCenterId."-".$workerId."\n";
        return SnowFlake::make($dataCenterId,$workerId);
    }

    public static function getLongNum() {
        $dataCenterId = config("dataCenterId");
...
//        }
    ...
    public static function checkToken($token,$address) {
        // ETH地址合法校验
    ...
    public static function validateEthAddress($value)
    {
        if (!is_string($value)) {
            return false;
        }
    ...
    public static function formatDispalyAddress($address) {

        return substr($address,0,5)."******".substr($address,strlen($address)-3);

        //return str_re
    }

    public static function retSucc($ret) {
    ...
    public static function formatSymbolDisplay($symbol,$exchange) {
        if($exchange == "okex" || $exchange=="okex_simulate") {
